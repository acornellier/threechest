...
// and rounded up.
//
//   kicksNeeded = ceil( sum over spawns of 15 / interval )
//
// The ceil handles simultaneity: two mobs on 18s intervals are only 1.67 "kicks worth" of rate,
// but they can cast at the same moment, so you still need two bodies.
//
...
/**
 * Interrupters required to cover an accumulated kick rate. The roundTo is load-bearing: summing
 * 15/interval across mixed intervals overshoots in floating point, so an exact 2 can land on
 * 2.0000000000000004 and ceil to 3.
 */
